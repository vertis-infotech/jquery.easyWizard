...
(function( $ ) {
  ...
  var direction = 'next';
	var easyWizardMethods = {
		...
		prevStep : function(animationDuration) {
      this.direction = 'prev';
      animationDuration = typeof animationDuration !== 'undefined' ? animationDuration : 200;
			...
			if($activeStep.prev('.'+thisSettings.stepClassName).length) {				
				...
				easyWizardMethods.goToStep.call(this, prevStep, animationDuration);
			}
		},
		nextStep : function(animationDuration) {
      this.direction = 'next';
      animationDuration = typeof animationDuration !== 'undefined' ? animationDuration : 200;
			thisSettings = arrSettings[this.index()];			
			$activeStep = this.find('.'+ thisSettings.stepClassName +'.active');
			if($activeStep.next('.'+thisSettings.stepClassName).length) {
				...
				easyWizardMethods.goToStep.call(this, nextStep, animationDuration);
			}
		},
		goToStep : function(step, animationDuration) {
      animationDuration = typeof animationDuration !== 'undefined' ? animationDuration : 200;
			thisSettings = arrSettings[this.index()];
			$activeStep = this.find('.'+ thisSettings.stepClassName +'.active');
